...
func sortString(s1, s2 string) (shorter, longer string) {
...
func Calculate(s1, s2 string) float64 {
	s1 = strings.ToLower(strings.TrimSpace(strings.ReplaceAll(s1, " ", "")))
	s2 = strings.ToLower(strings.TrimSpace(strings.ReplaceAll(s2, " ", "")))
	...
	if strings.HasPrefix(s2, s1) || strings.HasPrefix(s1, s2) {
		...
		return 0.85 + 0.15*(float64(len(shorter))/float64(len(longer)))
	}

	s1, s2 = sortString(s1, s2)
